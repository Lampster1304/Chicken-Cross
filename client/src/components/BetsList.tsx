        ...
        <div className="space-y-1 max-h-[320px] overflow-y-auto">
          {filteredFeed.map((entry, i) => {
            ...
            return (
              <div
                key={entry.id}
                className={`flex items-center justify-between px-3 py-2 rounded-xl text-[12px] hover:bg-bg-surfaceHover transition-colors ${
                  i === 0 ? 'animate-fade-in' : ''
              ...
              >
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {isWin ? (
                    <>
                      <span className={`font-semibold font-mono ${isBigWin ? 'text-amber-400' : 'text-success'}`}>{entry.multiplier?.toFixed(2)}x</span>
                      <span className={`text-[11px] font-mono ${isBigWin ? 'text-amber-400/70' : 'text-success/60'}`}>+${entry.profit?.toFixed(2)}</span>
                    </>
                  ) : (
