  ...
  return (
    <div className="min-h-screen bg-casino-dark text-white">
      ...
      <div className="max-w-4xl mx-auto p-4 space-y-6">
        {/* Explanation */}
        <div className="bg-casino-card border border-casino-border rounded-xl p-4 sm:p-6">
          ...
          <div className="text-gray-400 text-sm space-y-2">
            ...
            <p><strong className="text-white">1.</strong> A <span className="text-casino-accent">server seed</span> is generated and its SHA-256 hash is shown before you play.</p>
            <p><strong className="text-white">2.</strong> Each lane result: <code className="bg-casino-darker px-1 rounded">HMAC-SHA256(server_seed, client_seed:nonce:lane)</code> → roll % 100. If roll &lt; difficulty*20 → car.</p>
        ...
        <div className="bg-casino-card border border-casino-border rounded-xl p-4 sm:p-6">
          ...
          <div className="space-y-3">
            <div>
              ...
              <input
                ...
                className="w-full bg-casino-darker border border-casino-border rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-casino-accent font-mono"
              />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-casino-darker border border-casino-border rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-casino-accent"
                >
            ...
            <button
              onClick={verify}
              className="w-full sm:w-auto bg-casino-accent hover:bg-red-600 text-white font-bold px-6 py-3 rounded-lg transition min-h-[48px]"
            >
              Verify
            </button>
          </div>
          ...
          {result && (
            <div className="mt-4 bg-casino-darker border border-casino-border rounded-lg p-4 space-y-3">
              <div>
                <span className="text-gray-400 text-sm">
                  Per-lane multiplier: <span className="text-casino-green font-bold font-mono">{result.laneMultiplier.toFixed(2)}x</span>
                </span>
              </div>
              ...
              <table className="w-full text-xs sm:text-sm">
                ...
                <tbody>
                  {history.slice(0, 20).map(game => {
                    ...
                    return (
                      <tr key={game.id} className="border-b border-casino-border/30">
                        ...
                        <td className="py-2 px-2 text-right font-mono font-bold">
                          {isWin ? (
                            <span className="text-casino-green">{mult?.toFixed(2)}x</span>
                          ) : (
                            <span className="text-casino-accent">Hit L{game.current_lane}</span>
                          )}
                        </td>
                        <td className="py-2 px-2 text-right">
                          <button
                            onClick={() => fillFromHistory(game)}
                            className="text-casino-accent hover:underline text-xs"
                          >
                            Verify
                          </button>
                        </td>
