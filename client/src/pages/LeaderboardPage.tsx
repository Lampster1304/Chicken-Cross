...
export default function LeaderboardPage() {
  ...
  const getRankStyle = (rank: number) => {
    if (rank === 1) return 'text-casino-gold';
  ...
  return (
    <div className="min-h-screen bg-casino-dark text-white">
      ...
      <div className="max-w-4xl mx-auto p-4 space-y-4">
        {/* Period tabs */}
        <div className="flex gap-1 bg-casino-card rounded-lg p-1 border border-casino-border">
          {periods.map(p => (
            <button
              key={p.key}
        ...
        <div className="bg-casino-card border border-casino-border rounded-xl overflow-hidden">
          {loading ? (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {entries.map(entry => (
                  <tr key={`${entry.userId}-${entry.rank}`} className="border-b border-casino-border/30 hover:bg-casino-darker/50 transition">
                    ...
                    <td className="py-3 px-4 text-right font-mono font-bold text-casino-green">
